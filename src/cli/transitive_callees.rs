...
pub(crate) fn query_transitive_callees_cmd(
    ...
    let filtered: Option<Vec<GraphRecord>> =
        if matches!(corpus_mode, query::CorpusMode::CommitPinned) {
            ...
            // stamped HEAD BEFORE the walk, so a callee (or the CALLS edge) removed
            // at HEAD does not appear — not as reachable and not as unresolved
            // either, since the stale edge is dropped whole. Snapshot-less stores
            // never reach here. See `query::non_head_current_record_ids`.
