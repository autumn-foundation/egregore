...
#[derive(Serialize)]
struct ImplementorsZeroResult<'a> {
    ok: bool,
    code: &'static str,
    trait_record_id: &'a str,
    trait_name: &'a str,
...
fn exit_implementors_unresolved(
    code: &str,
    name: &str,
    at: Option<&str>,
    as_of: Option<&str>,
...
fn exit_implementors_non_target(name: &str, kinds: &[String]) -> ! {
    let envelope = serde_json::json!({
        "ok": false,
        "error": {
            "code": "no_match",
...
fn retain_implements_target_candidates(matches: &mut Vec<&GraphRecord>, name: &str) {
    if matches.is_empty() {
        return;
    }
...
fn cycle_display_path(cycle: &query::DependencyCycle<'_>) -> String {
...
fn parse_file_line_location(location: &str) -> std::result::Result<(&str, usize), &'static str> {
...
fn location_node_json(record: &GraphRecord) -> Option<LocationNodeJson<'_>> {
    let GraphRecord::Node {
        id,
        kind,
        schema_version,
...
fn location_error_exit(error: &serde_json::Value, exit_code: i32) -> ! {
    let envelope = serde_json::json!({ "ok": false, "error": error });
    println!(
        "{}",
