...
is DERIVED from new `NodeKind::ALL` / `EdgeLabel::ALL` constants, each pinned against an
INDEPENDENT oracle — `serde`'s unknown-variant error, regenerated by the derive macro from the
enum definition — plus a write-site test that `node_label(kind) == kind.as_str()` for every
kind, so it can never silently drift from the adapter (a guard that merely iterated `ALL` would
be circular and could not fail). The declarable keys are
...
write is reported under `observed.unknown_*` and NEVER scanned (the disclaimer says so, so a
passing verdict never implies it covered them). `checked` counts store ENTITIES: the adapter is
append-only, so an Egregore-superseded record version is a distinct live engine entity and IS
scanned — the count tracks `eg inspect --data-dir` physical totals, it does not dedupe.
Violations cite offending records by `codegraph_id`, resolved from upstream's engine-internal
`u64` sample ids; an unresolvable sample is COUNTED, never emitted, and citations are published
ONLY when the engine's sample is provably complete (upstream keeps just its first 16 offenders
per group in randomly-seeded map order, so a larger group's subset varies per process —
`sample_complete: false` then omits the ids rather than break byte-identical output). Every
store-read value (record handles, foreign labels, sidecar declarations) is control-sanitized
via the #104 helper before rendering — handles are never truncated, since a truncated handle
stops being a citation. Output is allow-list only
(labels, property keys, type tokens, counts, upstream reason strings, record handles — never
payload text, never an engine id) and byte-identical across runs. Exit 0 clean, 1 non-conforming or declaration refused (full report still printed either way),
2 usage/load — a read or scan failure is exit 2 with a stable stderr `code`, never exit 1, so a
CI gate is never told "violations found" for an I/O error. Conformance is a STRUCTURAL
...
0.2.0's other new opt-in subsystems (namespaces, property indexes, changefeed, encryption,
replication, multi-tenancy) are INERT — none is enabled. SCHEMA CONSTRAINTS are the one
exception: still OFF by default and never declared by any write path, but an operator can now
evaluate and declare them via `eg audit schema-constraints` (issue #486, above).
