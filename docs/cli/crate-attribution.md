...
- **Ambiguity** (`ambiguous_package_selector`) is silent-merge protection —
  ...
  corpus the answer is computed from**, at the current version of each record. A
  package two repositories held at different times, but only one holds at HEAD,
  is not ambiguous for a HEAD-anchored answer; refusing it would withhold an
  answerable result. `--all-history` widens the corpus, so the same query
  refuses there. Under `--at`/`--as-of` the corpus also excludes records with no
...
`cargo metadata --no-deps` and `cargo build` on the pinned toolchain, so
rejecting them would un-attribute crates Cargo builds happily. The one
exception is `cargo-features`, which Cargo *does* type-check (an array of
strings, "expected a sequence" otherwise) and which applies to **both** manifest
...
SHAPE is checked too — a spec must be a table and each of its dependency tables
a map — which is a real Cargo asymmetry rather than an oversight at the top
...
TEMPLATE rule, not the member one — Cargo accepts a source-less table, an empty
table, an unparseable version string, a `workspace` key, and unknown keys here,
while rejecting a non-string/non-table value, a wrong-typed known key,
`optional = true`, and an invalid dependency name. Cargo refuses to load such a manifest, so the package it names does not exist and cannot own the subtree. Every arm — accepted and rejected — was verified against real `cargo metadata`; the inheritance table is accepted everywhere Cargo accepts it, because rejecting it would un-attribute every workspace that inherits. As with `[workspace]`, the residual is VALUE-level validation: `version = "notsemver"` and `edition = "1066"` are Cargo errors this resolver does not evaluate. |
