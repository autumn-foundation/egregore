...
| `ambiguous_agent_provenance` | A session had more than one live `SESSION_OF` edge to a distinct `Agent` node — ingest validates endpoint kinds but not the documented many-to-one cardinality. `agent_record_id` reports `null` rather than guessing one; `candidate_ids` lists the competing `Agent` record IDs, capped at 20 (`count` carries the true total; `returned`/`limit` are set only when the list was actually truncated) |
